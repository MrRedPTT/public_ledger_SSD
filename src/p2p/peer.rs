...
impl PacketSending for Peer {
    async fn ping(&self, request: Request<PingPacket>) -> Result<Response<PongPacket>, Status> {
        ...
        println!("Test of near nodes: {:?}", self.kademlia.lock().unwrap().get_k_nearest_to_node(self.node.id.clone()).unwrap().len());
        if input.src.is_none() || input.dst.is_none() {
            // Abort!
        }
        let args = <Option<Address> as Clone>::clone(&input.dst).unwrap(); // Avoid Borrowing
        if args.ip != self.node.ip || args.port != self.node.port || args.id != self.node.id.0.to_vec() {
            // Abort!
        }
    ...
    async fn find_node(&self, request: Request<FindNodeRequest>) -> Result<Response<FindNodeResponse>, Status> {
        ...
        let mut kademlia_ref = &mut *self.kademlia.lock().unwrap();
        ...
        if lookup_src_node.is_none() {
            ...
            kademlia_ref.add_node(Node::new(src.ip, src.port).unwrap());
        ...
        if dst.ip != self.node.ip || dst.port != self.node.port || dst.id != self.node.id.0.to_vec() {
            let response = FindNodeResponse {
                response_type: 1, // UNKNOWN_TYPE => Abort this communication
                node: placeholder_node.clone(),
                list: Self::return_option(proto::KNearestNodes{nodes: Vec::new()}),
                error: format!("The supplied destination is invalid!\nYou passed: {}:{} id = {:?}\nExpected: {}:{} id = {:?}", dst.ip, dst.port, dst.id, self.node.ip, self.node.port, self.node.id.0.to_vec()).to_string()
            };
            return Ok(tonic::Response::new(response));
        }
